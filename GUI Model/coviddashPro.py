...
def load_data():
    ...
    latest=covid[covid["Date"] == "2021-08-02"][["Country","Confirmed","Recovered","Deaths","Active"]]
    return covid,latest
covid,latest= load_data()



st.title('🦠 Covid-19 Disease Dynamics 🦠 ')
st.sidebar.markdown('🦠 **Covid-19 Dashborad** 🦠 ')
st.sidebar.markdown(''' 
Covid-19 Disease Dynamics project is based on the analysis of Covid-19 infection around the world.

The data considerd for this analysis is during 01-02-2020 to 02-08-2021


Features - Analysing Upcoming Covid Peak, Interactive Charts, Time Series Analysis, DayByDay Analysis 
...
"From 01-02-2020 to 02-08-2021"
tot = latest[latest["Country"]==cty]['Confirmed'].sum()

#st.subheader(f"Total Confirmed cases in {country} = {tot}")

reco = latest[latest["Country"]==cty]['Recovered'].sum()

#st.subheader(f"Total Recovered in {country} = {reco}")

act = latest[latest["Country"]==cty]['Active'].sum()

#st.subheader(f"Total Active cases in {country} = {act}")

dths = latest[latest["Country"]==cty]['Deaths'].sum()

#st.subheader(f"Total Deaths occured in {country} = {dths}")
...
import datetime
ddd = st.date_input(
     "Select the Date",value = datetime.date(2020, 2, 1),min_value = datetime.date(2020, 2, 1), max_value= datetime.date(2021, 8, 2))
